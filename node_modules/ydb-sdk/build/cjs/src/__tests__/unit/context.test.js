...
// @ts-ignore
const symbols_1 = require("../../context/symbols");
describe('Context', () => {
    beforeEach(() => {
        (0, context_1.setContextIdGenerator)(); // back to default
    });
    afterEach(() => {
        jest.useRealTimers();
    });
    it('id generation', () => {
        expect(context_1.Context.createNew().ctx.id).toBe('0001');
        expect(context_1.Context.createNew().ctx.toString()).toBe('0002');
        (0, context_1.setContextIdGenerator)(() => '');
        expect(context_1.Context.createNew().ctx.id).toBe('');
        expect(context_1.Context.createNew().ctx.id).toBe('');
    });
    it('simple', () => {
        const { ctx: ctx1, cancel: cancel1, dispose: dispose1, done: done1 } = context_1.Context.createNew();
        ...
        const { ctx: ctx2, cancel: cancel2, dispose: dispose2, done: done2 } = ctx1.createChild();
    ...
    for (const mode of [
        ...
        it(`cancel; mode: ${mode}`, () => {
            const { ctx: ctx1, cancel: cancel1, dispose: dispose1, done: done1 } = context_1.Context.createNew({ cancel: true });
            expect(ctx1).toBeDefined();
            ...
            expect(dispose1).toBeUndefined();
            expect(done1).toBeUndefined();
            const { ctx: ctx2, cancel: cancel2, dispose: dispose2, done: done2 } = ctx1.createChild({ cancel: true });
            expect(ctx2).toBeDefined();
            ...
            expect(done2).toBeUndefined();
            const { ctx: ctx3, cancel: cancel3, dispose: dispose3, done: done3 } = ctx1.createChild({ force: true });
            ...
            expect(ctx1[symbols_1.cancelListenersSymbol]).toHaveLength(3); // two sub-ctx and one onCancel
            expect(ctx2[symbols_1.cancelListenersSymbol]).toHaveLength(1); // one onCancel
            expect(ctx3[symbols_1.cancelListenersSymbol]).toHaveLength(1); // one onCancel
            switch (mode) {
                case 'cancel1':
                    {
                        if (cancel1)
                        ...
                        expect(cnt2).toBe(1);
                        expect(cnt3).toBe(1);
                        expect(ctx1[symbols_1.cancelListenersSymbol]).toBeUndefined();
                        expect(ctx1.err).toBe(testCancel);
                        expect(ctx2[symbols_1.cancelListenersSymbol]).toBeUndefined();
                        expect(ctx2.err).toBe(testCancel);
                        expect(ctx3[symbols_1.cancelListenersSymbol]).toBeUndefined();
                ...
                case 'cancel2':
                    {
                        ...
                        expect(cnt2).toBe(1);
                        expect(cnt3).toBe(0);
                        expect(ctx1[symbols_1.cancelListenersSymbol]).toHaveLength(3);
                        expect(ctx1.err).toBeUndefined();
                        expect(ctx2.hasOwnProperty(symbols_1.cancelListenersSymbol)).toBeFalsy();
                        expect(ctx2.err).toBe(testCancel);
                        expect(ctx3[symbols_1.cancelListenersSymbol]).toHaveLength(1);
                        expect(ctx3.err).toBeUndefined();
                    }
                    break;
                case 'cancel3':
                ...
                case 'unsub':
                    {
                        ...
                        expect(ctx1[symbols_1.cancelListenersSymbol]).toHaveLength(2); // two listening child ctx
                        expect(ctx2[symbols_1.cancelListenersSymbol]).toHaveLength(0);
                        expect(ctx3[symbols_1.cancelListenersSymbol]).toHaveLength(0);
                    }
                    break;
                case 'dispose':
                    {
                        expect(ctx1[symbols_1.cancelListenersSymbol]).toHaveLength(3); // one onCancel and two listening child ctx
                        if (dispose2)
                            dispose2(); // dispose ctx2
                        expect(ctx1[symbols_1.cancelListenersSymbol]).toHaveLength(2); // one onCancel and one listening child ctx
                        if (dispose2)
                            dispose2(); // // second time call won't affect
                        expect(ctx1[symbols_1.cancelListenersSymbol]).toHaveLength(2); // one onCancel and one listening child ctx
                        if (dispose3)
                            dispose3(); // dispose ctx3
                        expect(ctx1[symbols_1.cancelListenersSymbol]).toHaveLength(1); // one onCancel
                        unsub1();
                        expect(ctx1[symbols_1.cancelListenersSymbol]).toHaveLength(0); // nothing
                        unsub1(); // second time call won't affect
                    }
                    break;
                case 'timeout':
                    {
                        const { ctx: _ctx4, dispose: dispose4 } = ctx1.createChild({ timeout: 2000 });
    ...
    for (const mode of [
        ...
        it(`dispose with timeout; mode: ${mode}`, () => {
            jest.useFakeTimers();
            const { ctx: ctx1 } = context_1.Context.createNew({ cancel: true });
            // dispose2 whould combine cancel timer and unsubscribe from ctx1.onCancel
            const { ctx: ctx2, dispose: dispose2 } = ctx1.createChild({ timeout: 2000 });
            let cnt = 0;
            ctx2.onCancel((cause) => {
                cnt++;
                expect(context_1.Context.isTimeout(cause)).toBeTruthy();
            });
            switch (mode) {
    ...
    for (const mode of [
        ...
        it(`timeout; mode: ${mode}`, () => {
            jest.useFakeTimers();
            const { ctx, cancel, dispose, done } = context_1.Context.createNew({
                timeout: 2000,
            });
                ...
                case 'general':
                    {
                        ...
                        ctx.onCancel((cause) => {
                            cnt++;
                            expect(context_1.Context.isTimeout(cause)).toBeTruthy();
                            expect(context_1.Context.isDone(cause)).toBeFalsy();
                        });
                        expect(cnt).toBe(0);
                        ...
                        jest.advanceTimersByTime(1000);
                        expect(cnt).toBe(1);
                        expect(context_1.Context.isTimeout(ctx.err)).toBeTruthy();
                        jest.advanceTimersByTime(1000);
                        expect(cnt).toBe(1);
                    }
                    break;
    ...
    it('done', () => {
        const { ctx, done } = context_1.Context.createNew({
            done: true,
        });
        expect(ctx.onCancel).toBeDefined();
        let cnt = 0;
        ctx.onCancel((cause) => {
            cnt++;
            expect(context_1.Context.isDone(cause)).toBeTruthy();
            expect(context_1.Context.isTimeout(cause)).toBeFalsy();
        });
        expect(cnt).toBe(0);
        done();
        expect(cnt).toBe(1);
        expect(context_1.Context.isDone(ctx.err)).toBeTruthy();
    });
    it('values', () => {
        const Symbol1 = Symbol('a');
        const Symbol2 = Symbol('b');
        const { ctx: ctx1 } = context_1.Context.createNew();
        ctx1[Symbol1] = 'aaa';
    ...
    it('break cancel-chain', () => {
        const Symbol1 = Symbol('a');
        const { ctx: ctx1, cancel: cancel1 } = context_1.Context.createNew({ cancel: true });
        ctx1[Symbol1] = 'aaa';
    ...
    it('make 100% coverage', () => {
        {
            const { ctx } = context_1.Context.createNew({ id: 'test' });
            expect(ctx.id).toBe('test'); // context.id might be set for new ctx
        }
        {
            const { ctx, cancel } = context_1.Context.createNew({ cancel: true });
        ...
        {
            const { cancel } = context_1.Context.createNew({ cancel: true });
            cancel();
