...
async function createTable(session) {
    await session.dropTable(test_1.TABLE);
    await session.createTable(test_1.TABLE, new table_1.TableDescription()
...
REPLACE INTO ${test_1.TABLE}
SELECT * FROM AS_TABLE($data);`;
    await (0, retries_obsoleted_1.withRetries)(async () => {
...
describe('bytestring identity', () => {
    ...
    beforeAll(async () => {
        ...
        await driver.tableClient.withSession(async (session) => {
            ...
            const { resultSets } = await session.executeQuery(`SELECT * FROM ${test_1.TABLE}`);
